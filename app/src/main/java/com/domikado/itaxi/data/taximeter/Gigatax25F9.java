...
public class Gigatax25F9 extends Gigatax {

    private final static String HEXES = "0123456789ABCDEF";
    private SerialBuffer mSerialBuffer = new SerialBuffer(true);
    private final static int TOTAL_BYTES = 30;
    private final static String START_BYTE = "0xAA";
    ...
    public Gigatax25F9(EventBus bus) {
        this.bus = bus;
    }
    ...
    private BigDecimal getFareAmount(byte[] data) {
        StringBuilder fare = new StringBuilder(8);
        fare.append(HEXES.charAt((data[16] & 0xF0) >> 4)).append(HEXES.charAt((data[16] & 0x0F)));
