...
public class LogUploader {
    ...
    public Observable<ResponseBody> call() {
        setRunning(true);
        String url = TaxiApplication.getSettings().getUploadLogUrl();
        ...
        return listFiles()
            ...
            .doOnNext(response ->
                Hawk.put(Constant.DataStore.LOG_LAST_UPLOAD, new RequestHistory.Builder()
                    .setUrl(url)
    ...
    private Observable<File[]> listFiles() {
        return Observable.fromEmitter(emitter -> {
            try {
                File dir = FileUtils.getDataDir(Constant.DataStore.LOG_DIR);
                File[] files = dir.listFiles(file -> file.isFile() && file.getName().startsWith("archive"));
                if (files != null) {
